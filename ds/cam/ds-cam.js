module.exports = function(RED) {
	var ssReferences = require('./ds-surveillance-references.js');
	var dsUtils = require('./../ds-utils.js');

	/**********/
	// DS Cam //
	/**********/
	function DS_Cam_Node(config) {
		...
		if (this.dsmNode) {
			...
			this.on('input', function(msg) {
				...
				node.dsmNode.syno.ss[topic](payload, function(error, data) {
					if( error )
						node.error(error);
					else {
						msg.payload = data;
	...
	RED.nodes.registerType("DS Cam", DS_Cam_Node);

	/* ---------------------------------------------------------------------------
	 * Backend informations
	 * -------------------------------------------------------------------------*/
	dsUtils.provideReferences(RED, 'surveillancestation', ssReferences);
};